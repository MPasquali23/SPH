...
# ======================================================================
# 12b. CHECKPOINT SAVE / LOAD  (NPZ for restart)
# ======================================================================
...
def save_checkpoint(step, t, hw, Sn, time_log, l2_log, l2n_log,
    ...
    np.savez_compressed(path,
        ...
        l2n_log=np.array(l2n_log),
        Sn_max_log=np.array(Sn_max_log),
...
def load_latest_checkpoint():
    """Load most recent checkpoint. Returns None if no checkpoint found."""
    import glob
    files = sorted(glob.glob(os.path.join(CKPT_DIR, "ckpt_*.npz")))
    if not files:
        return None
    path = files[-1]
    d = np.load(path, allow_pickle=True)
    print(f"  Restarting from checkpoint: {path}")
    print(f"    step = {int(d['step'])},  t = {float(d['t_phys']):.4e} s")
    return d
